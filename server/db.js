...
const sequelize = new Sequelize(`postgres://${process.env.cloudUser}:${process.env.cloudPW}@${process.env.cloudURL}:${process.env.DB_PORT}/${process.env.DB_NAME}`, {
  logging: false,
});
...
const getQAbyProductId2 = (product_id, page, count) => {
  ...
  return sequelize.query(`SELECT "Questions".question_id AS question_id, "Questions".question_body AS question_body, "Questions".question_date AS question_date, "Questions".asker_name, "Questions".question_helpfulness AS question_helpfulness, "Answers".answer_id AS answer_id, "Answers".answer_body AS answer_body, "Answers".answer_date AS answer_date, "Answers".answerer_name, "Answers".answer_helpfulness AS answer_helpfulness, "Photos".url
  FROM (SELECT * FROM "Questions" WHERE product_id = ${product_id} AND "Questions".question_reported = false ORDER BY "Questions".question_id LIMIT ${limit} OFFSET ${offset}) "Questions" LEFT OUTER JOIN "Answers" ON "Questions".question_id = "Answers".question_id AND ("Answers".answer_reported = false OR "Answers".answer_reported = null) LEFT OUTER JOIN "Photos" ON "Answers".answer_id = "Photos".answer_id;`, {
...
const getAnswersByQuestionId2 = (question_id, page, count) => {
  ...
  const limit = count;
  return sequelize.query(`SELECT "Answers".answer_id AS answer_id, "Answers".answer_body AS answer_body, "Answers".answer_date AS answer_date, "Answers".answerer_name, "Answers".answer_helpfulness AS answer_helpfulness, "Photos".photo_id AS photo_id, "Photos".url from (SELECT * from "Answers" WHERE "Answers".question_id = ${question_id} AND "Answers".answer_reported = false ORDER BY "Answers".answer_id limit ${limit} OFFSET ${offset}) "Answers" LEFT OUTER JOIN "Photos" ON "Answers".answer_id = "Photos".photo_id;`, {
    type: QueryTypes.select,
  })
    .then((res) => {
      let j = 0;
